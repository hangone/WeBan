...
def test_startup_import_failure_keeps_interactive_window_open(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, non_interactive: bool
) -> None:
    runtime = _runtime(tmp_path, non_interactive=non_interactive)
    monkeypatch.setattr(main, "_build_runtime", lambda *args: (None, {}, runtime, False))
    prompts: list[str] = []
    ...
    def fail_import(*args: object) -> None:
        raise ModuleNotFoundError("No module named 'nodriver'")

    monkeypatch.setattr("builtins.input", read_input)
    monkeypatch.setattr(main, "_apply_runtime_adapters", fail_import)

    assert main.main([], env={}) == main.EXIT_FAILURE
    if non_interactive:
        assert prompts == []
    else:
        assert prompts[-1] == "按回车键退出"
    log_text = next(runtime.paths.logs_dir.glob("*.log")).read_text(encoding="utf-8")
    assert "nodriver" in log_text
