...
@pytest.mark.parametrize("cancel_during_navigation", [False, True])
def test_build_page_uses_origin_and_restores_local_storage(
    monkeypatch: pytest.MonkeyPatch,
    entry_url: str,
    initial_origin: str,
    cancel_during_navigation: bool,
) -> None:
    class FakeTab:
        def __init__(self) -> None:
    ...
    async def scenario() -> None:
        if cancel_during_navigation:
            with pytest.raises(asyncio.CancelledError):
                await handler._build_page(entry_url, headless=True)
            assert handler._browser_states == {}
            return
...
def test_atomic_debug_write_cleans_temporary_file_on_failure(
    ...
    monkeypatch.setattr(captcha.cv2, "imencode", lambda *args, **kwargs: (False, None))
...
def test_real_login_model_loads_from_unicode_directory(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, frozen: bool
) -> None:
    model_bytes = Path(captcha.__file__).with_name("captcha_model.onnx").read_bytes()
    model_dir = tmp_path / "中文模型目录"
    model_dir.mkdir()
    (model_dir / "captcha_model.onnx").write_bytes(model_bytes)
    monkeypatch.setattr(captcha, "__file__", str(model_dir / "captcha.py"))
    monkeypatch.setattr(captcha.sys, "frozen", frozen, raising=False)
    monkeypatch.setattr(captcha.sys, "_MEIPASS", str(model_dir), raising=False)
    monkeypatch.setattr(captcha.LoginCaptchaSolver, "_initialized", False)
    monkeypatch.setattr(captcha.LoginCaptchaSolver, "_ocr", None)
    log = StubLog()

    net = captcha.LoginCaptchaSolver.get_ocr(log)
    assert net is not None, log.messages
    net.setInput(np.zeros((1, 1, 28, 28), dtype=np.float32))
    output = net.forward()
    assert output.shape == (1, 36)
    assert np.isfinite(output).all()


def test_atomic_debug_png_roundtrips_in_unicode_directory(tmp_path: Path) -> None:
    destination = tmp_path / "中文调试目录" / "验证码.png"
    pixels = np.arange(36, dtype=np.uint8).reshape(3, 4, 3)

    captcha._write_png_atomic(destination, pixels)

    decoded = captcha.cv2.imdecode(
        np.frombuffer(destination.read_bytes(), dtype=np.uint8),
        captcha.cv2.IMREAD_COLOR,
    )
