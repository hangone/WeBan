    ...
    *,
    ...
    try:
        ...
        if runtime.interaction.allow_input:
            choice = (
                input(f"当前已配置 {len(runtime.accounts)} 个账号，是否更换账号？(y/N)：")
                .strip()
                .lower()
            )
            if choice == "y":
        ...
        base_logger.remove()
        # 交互启动失败也应保留窗口，让用户看到错误；无交互运行和
        # 已请求中断的任务不等待输入。清理先完成，避免等待时占用资源。
        if runtime.interaction.allow_input and not stop_event.is_set():
            try:
                input("按回车键退出")
