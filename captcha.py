                ...
                if not cls._initialized:
                    try:
                        ...
                        if not model_path.exists():
                            ...
                            # Windows 下 OpenCV 的文件接口不能可靠处理中文路径。
                            # 由 Python 读取字节，兼容源码目录和冻结程序解包目录。
                            model_buffer = np.frombuffer(
                                model_path.read_bytes(), dtype=np.uint8
                            )
                            cls._ocr = cv2.dnn.readNetFromONNX(model_buffer)
...
class CaptchaHandler:
    ...
    async def _build_page(self, entry_url: str, headless: bool = False):
        ...
        try:
            ...
            # 外层流程超时会取消本协程；CancelledError 也必须先恢复共享
            # 状态并关闭本次浏览器，再把取消传播给调用方。
